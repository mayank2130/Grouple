    ...
    >
      {userGroups &&
        ...
        userGroups.map((item) => (
          <Link
            key={item.id}
            href={`/group/${item.id}/channel/${item.channel && item.channel[0] ? item.channel[0].id : ""}`}
          >
            <Button
              variant="ghost"
      ...
      {members &&
        ...
        members.map((member) => (
          <Link
            key={member.Group?.id}
            href={`/group/${member.Group?.id}/channel/${member.Group?.channel && member.Group.channel[0] ? member.Group.channel[0].id : ""}`}
          >
            <Button
              variant="ghost"
              className="flex gap-2 w-full justify-start hover:bg-themeGray items-center"
